...
h3("Bespoke Interface Theme")
p("The interface originally used Gradio's default styling, which is functional but "
  "generic and gives no visual identity to the application. A bespoke theme was "
  "designed and applied instead, built on Gradio's theming API rather than custom "
  "HTML, so that every built-in component, such as buttons, sliders, checkboxes and "
  "accordions, still behaves and responds exactly as Gradio intends while presenting a warm, "
  "parchment-toned colour palette (a cream background, a deep teal accent colour for "
  "primary actions and links, and a muted amber border colour) together with a "
  "serif heading font for the title and a system sans-serif font for body text. A "
  "short gradient header banner introduces the application by name, states in a "
  "single line what it does, and carries a badge reading 'Runs entirely offline; "
  "nothing leaves this machine', restating the project's offline-first principle "
  "directly in the interface rather than leaving it only in the documentation. The "
  "upload, settings and conversation controls are each wrapped in a visually "
  "distinct card-like group so the page reads as a small number of clear sections "
  "rather than one undifferentiated column of controls, which was judged more "
  "in keeping with a minimalist, uncluttered design than adding further visual "
  "decoration would have been.")
p("Because every font referenced by the theme had to resolve locally rather than "
  "fetch a webfont from a remote CDN, consistent with the project's offline-only "
  "requirement (Section 5), the theme is built entirely from system font stacks "
  "(Segoe UI, Georgia and their generic fallbacks) rather than Gradio's optional "
  "Google Fonts integration. Building the theme surfaced two problems that would "
  "not have been visible from reading the theming API documentation alone: passing "
  "plain strings instead of Gradio's Font wrapper objects for the font stack caused "
  "a crash on startup when Gradio internally compared the custom theme against its "
  "built-in themes, and leaving the theme's dark-mode colour variants unset let a "
  "visitor's operating system colour preference silently override the intended "
  "palette with Gradio's default dark colours wherever a dark-mode counterpart had "
  "not been explicitly given. Both were fixed, the first by wrapping every font "
  "name in a Font object, the second by setting every dark-mode variable to the "
  "same value as its light counterpart so the theme renders identically regardless "
  "of the viewer's system preference, and the fix for each was confirmed by "
  "reloading the running application in the browser and taking a fresh screenshot. "
  "A third, more subtle issue was traced the same way: an unset border colour used "
  "internally to fill the empty space between grouped controls was resolving to a "
  "near-black default, producing a plain-looking dark block above the chat window "
  "and to the side of the document controls; the running page's own computed "
  "styles were inspected directly in the browser to identify which theme variable "
  "was responsible, and setting it to match the rest of the palette resolved it. "
  "The full test suite was re-run after these changes to confirm the styling work "
  "had not affected any pipeline or ingestion behaviour, and the core user flows, "
  "uploading and removing documents, asking and regenerating a question, saving "
...
log_entries = [
    ...
    ("Week 26", "Early September",
     ...
     "chunk-count and replaced-document messaging. Three new unit tests were added "
    ...
    ("Week 27", "Early September",
     ...
     "normal behaviour. Two problems only appeared once the theme was actually run "
     "in the browser rather than merely read as code: passing plain strings for the "
     "font stack crashed the application on startup, because Gradio compares a "
     "custom theme's fonts against its built-in themes' Font objects when deciding "
     "whether the theme is custom, and a dark-mode colour preference on the test "
     "machine silently overrode every theme colour that had no explicit dark-mode "
     "counterpart set. Both were fixed and confirmed by reloading the running page "
     "and taking a screenshot after each change. A further, more subtle visual bug, "
     "a plain dark block appearing above the chat window, was traced by inspecting "
     "the running page's own computed styles directly in the browser rather than "
     "guessing from the theme code, which identified an unset border colour as the "
     "cause; the same technique was reused a moment later to fix an unstyled dark "
     "checkbox once the first fix revealed the pattern. The full unit test suite "
     "was re-run to confirm the purely visual changes had not affected any "
     "pipeline or ingestion behaviour, and every core user flow, uploading and "
     "removing documents, asking and regenerating an answer, saving and resuming a "
     "session, and switching model and answer-style settings, was re-exercised "
     "live in the browser against the themed interface. What went well: reproducing "
